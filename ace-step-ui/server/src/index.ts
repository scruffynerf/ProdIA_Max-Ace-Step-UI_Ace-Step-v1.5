...
app.post('/api/server/restart', (_req, res) => {
  ...
  // Exit with code 75 (a non-error code we use to signal "restart wanted")
  // The start script or process manager should detect this and restart.
  // As a fallback, we spawn a new instance of ourselves before exiting.
  setTimeout(async () => {
    try {
      ...
      child.unref();
      console.log('[server] Spawned new instance, shutting down current...');
    } catch (err) {
      console.error('[server] Failed to spawn restart process:', err);
    }
    process.exit(0);
  }, 500);
});
