...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const defaultShareText = `🎵 "${song.title}" ${song.style ? `(${song.style})` : ''} - Made with ProdIA pro`;
  const twitterText = `🔥 Just created "${song.title}" with ProdIA pro - local AI music generation! ${song.style ? `#${song.style.replace(/\s+/g, '')}` : ''} #AIMusic #ProdIA`;
  const redditTitle = `[AI Music] ${song.title} - ${song.style || 'Original'} | Created with ProdIA pro`;
  const whatsAppText = `🎧 Listen to this AI-generated song!\n\n"${song.title}" by ${song.creator || 'Unknown Artist'}\n${song.style ? `Genre: ${song.style}` : ''}\n\nMade with ProdIA pro - free and open source!`;
